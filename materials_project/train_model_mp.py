...
train_dataset = tf.data.TFRecordDataset('tfrecords_mp/train.tfrecord.gz', compression_type='GZIP')\
...
valid_dataset = tf.data.TFRecordDataset('tfrecords_mp/valid.tfrecord.gz', compression_type='GZIP')\
    .map(parse_example, num_parallel_calls=tf.data.experimental.AUTOTUNE)\
    .cache()\
...
site_class = layers.Input(shape=[None], dtype=tf.int64, name='site_features')
...
# dimension=10,
rbf_distance = RBFExpansion(dimension=128,
                            init_max_distance=7,
                            init_gap=30,
...
STEPS_PER_EPOCH = math.ceil(45633 / batch_size)  # number of training examples
lr = 1E-4
...
model_name = 'trained_model_mp_nodropout_large_1E3'
